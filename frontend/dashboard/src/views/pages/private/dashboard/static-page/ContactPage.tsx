...
import {
  getContactPageItemData,
  getContactPageFormData,
} from "@/utils/constants/dashboard/contact-constants";
...
const ContactPage = () => {
  ...
  const onSubmit = (data: any) => {
    ...
    const modifiedData = getContactPageFormData(data, item); // get the correct data pattern here
    const formData = {
  ...
  return (
    <comp.StaticPageForm {...staticPageProps}>
      <div className="col-span-12 md:col-span-7">
        <comp.ColoredDivSection status="secondary" title="Common">
          {inputFields?.map((singleField: string, idx: number) => {
            // Variables
            const isTextArea = ["caption", "inquiry"]?.includes(singleField);
            const textAreaHeight =
              isTextArea && singleField === "caption"
                ? "min-h-[100px]"
                : "min-h-[270.5px]";
            return (
      ...
      <div className="col-span-12 md:col-span-5 flex flex-col gap-8">
        {inputBranches?.map((singleBranch: string, idx: number) => (
          <comp.ColoredDivSection
            key={idx}
            title={toTitleCase(singleBranch)}
            status="secondary"
          >
            {["phone", "email", "location"]?.map(
              (info: string, idx: number) => {
                const name = `${singleBranch}_${info}`;
                return (
                  <comp.ControllerInput
                    key={idx}
                    {...f.title({
                      control,
                      name,
                      label: toTitleCase(info),
                    })}
                    {...getInputErr(errors, name)}
                    value={formData[name] ?? item?.[singleBranch]?.[info]}
                    onChange={(e: InputOnChangeSchema) =>
