...
const DeleteSubcategoryDraft = () => {
  ...
  const m = {
    method: type,
    route: CRUD_ROUTES({ id, model: "subcategory_draft" }).delete,
    onSuccess: (data: any) => {
      setToast(t.TOAST_SUCCESS(getApiSuccessMsg(data)));
      subcategoryApi.subgetAllRefetch();
      subcategoryApi.subDraftGetAllRefetch();
...
export default DeleteSubcategoryDraft;
