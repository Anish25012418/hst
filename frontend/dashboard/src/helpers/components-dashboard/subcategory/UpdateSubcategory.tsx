...
const UpdateSubcategory = () => {
  ...
  const { getAllRefetch, subgetAllRefetch } = useSubcategoryApi();
  ...
  const m = {
    method: "put",
    route: SUB_CATEGORY_ROUTES({ id: item?._id }).update,
  ...
  const [
    watchImageCoverPic,
    watchImageGalleryPic,
  ] = watch([
    "imageCoverPic",
    "imageGalleryPic",
  ]);

  const isImageCoverChanged = JSON.stringify(watchImageCoverPic ?? []) !== JSON.stringify(item?.imageCoverPic ?? []);
  const isGalleryChanged = JSON.stringify(watchImageGalleryPic ?? []) !== JSON.stringify(item?.imageGalleryPic ?? []);
  ...
  const onSubmit = (data: any) => {
    ...
    const subcategoryData = fetchSubcategoryData(data, item, itinerary);
    const { formData, imageValidity } = subcategoryData;
    ...
    console.log("DEPARTURE - data:", data?.departureFrom);
    console.log("DEPARTURE - item:", item?.departureFrom);
    console.log("subsetCheck:", isObjectSubsetOf({ ...data, itinerary }, item));

    const finalFormData = {
      ...(formData),
      ...(isImageCoverChanged && { imageCoverPic: imageCoverPicTemp }),
      ...(isGalleryChanged && { imageGalleryPic: imageGalleryPicTemp }),
    }
  ...
  // Custom sticky header props
  const stickyHeaderProps = {
    rest,
    title: isLessThan600 ? "Edit" : "Edit Subcategory",
    submitTitle: "Update",
    isLoading,
    type,
    setForm,
    isSubmitDisabled: false,
    liveUrl: getWebUrl(`${item?.categoryIds?.[0]?.slug}/${item?.slug}`),
  };
            ...
            <comp.ColoredDivSection title="Images" status="secondary">
              ...
              <comp.ControllerInput
                {...f.imageInput({
                  ...
                  label: "Gallery image",
                  control,
                  defaultFiles: item?.imageGalleryPic ?? [],
                })}
                imageList={
